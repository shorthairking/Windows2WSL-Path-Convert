...
> 2026-08-27 与用户确认，以下三项为最终决策，开发代理不得自行变更：

1. ✅ 核心引擎语言：**Rust**（零依赖单二进制）。
2. ✅ 拦截机制：**bash DEBUG trap hook**（脚本场景用 `wpc` CLI 显式转换，不做 LD_PRELOAD）。
3. ✅ 服务状态落地：**bashrc hook 自动激活 + systemd user 占位单元**（不实现常驻 daemon）。
...
2. **UNC 路径**：匹配 `\\` 开头且后跟合法的服务器名起始字符（非 `\`、空白、引号或通配符 `*?[`）；识别后标记为错误类别 `UnsupportedUnc`（不输出转换结果）。转义/glob 形态（如 `\\*`、补全函数中的 `[[ $cmd == \\* ]]`）不视为 UNC。
...
1. **捕获**：DEBUG trap 中读 `$BASH_COMMAND`（交互式下为原始命令行文本）。
   - **补全上下文防护**：TAB 补全期间 bash 会设置 `COMP_LINE`/`COMP_POINT`，DEBUG trap 也会对补全函数内部命令（如 `[[ $cmd == \\* ]]`）触发；这些不是用户真实命令行，故 `COMP_LINE`/`COMP_POINT` 已设置时 hook 直接跳过转换（不 fork、不拦截）。
